...
function start (files, command) {
  ...
  server.on('upgrade', function(request, socket, body) {
    ...
    if (WebSocket.isWebSocket(request)) {
      ...
      var options = JSON.stringify(command);

      ws.send(JSON.stringify({ start: true, options: options, url: customFrameURL || '/run' }));
  ...
  function onMessage (msg) {
    ...
    if (msg.result) {
      outputResult(msg, command.tap);
      if (command.quit) process.exit(msg.result.failed);
      return;
    }

    if (msg.fail) {
      if (!command.tap || command.stopOnFirstFailure) {
        outputError(msg, command.tap);
      }
      if (command.stopOnFirstFailure) {
        console.error("Stopping on first failed test because stopOnFirstFailure flag is set");
        process.exit(1);
      }
      return;
    }
...
function outputError (msg, outputRawTAP) {

  if (outputRawTAP) {
    console.log(msg.fail.rawTAPOutput);
  } else {
    var fail = msg.fail;
    var error = {
      message: fail.name,
...
function outputResult (msg, outputRawTAP) {

  if (outputRawTAP) {
    console.log(msg.result.rawTAPOutput);
  } else {
