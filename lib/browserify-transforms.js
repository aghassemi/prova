...
module.exports = function(files, command) {
  ...
  if (command.globalTransform && command.globalTransform.length) {
    command.globalTransform.split(',').forEach(function(name) {
      if (!name) return;
      debug('Global transform "%s" enabled', name);
      ret.transform(name, {global: true});
    });
  }
